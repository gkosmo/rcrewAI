  ...
  describe 'DSL on Tools::Base subclass' do
    let(:tool_class) do
      Class.new(RCrewAI::Tools::Base) do
        ...
        def execute(query:, max_results: 10, tags: [], verbose: false, mode: 'fast') # rubocop:disable Lint/UnusedMethodArgument
  ...
  describe '#execute_with_validation' do
    let(:tool_class) do
      Class.new(RCrewAI::Tools::Base) do
        ...
        def execute(n:) = n * 2 # rubocop:disable Naming/MethodParameterName
      end
    end
