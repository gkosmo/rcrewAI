...
- Knowledge (RAG): the new `RCrewAI::Knowledge` module adds retrieval-augmented context. Sources (`StringSource`, `FileSource`, `PdfSource`, `CsvSource`, `UrlSource`) are chunked, embedded, and stored in an in-memory cosine-similarity vector store (no external DB required). Attach via `Agent.new(knowledge:)` / `knowledge_sources:` (role-specific) or `Crew.new(knowledge:)` / `knowledge_sources:` (shared with all agents); relevant chunks are injected into each task's prompt at execution. The embedder (`Knowledge::Embedder`, default OpenAI `text-embedding-3-small`) and vector store are pluggable. (#9)
- Flows: `RCrewAI::Flow` is an event-driven workflow engine (CrewAI's second pillar). Subclass it and declare methods with a class-level DSL — `start`, `listen`, `router`, and the `and_` / `or_` trigger combinators. `kickoff` runs the graph to a fixed point; routers emit labels that listeners trigger on. Flow state is a schemaless object with an automatic UUID, seedable via `kickoff(inputs:)`, and persistable/restorable through a state store (`Flow::MemoryStateStore`, `Flow::FileStateStore`, or any `#save`/`#load` object). Flows can invoke a Crew as a step and pause for input via `#human_feedback`. (#11)
